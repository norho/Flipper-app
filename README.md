...
1. Select **"Scrivi tag UFUID"** from the menu.
2. Select your `.bin` file.
3. The app will automatically unlock the tag, write the data block-by-block, and complete the process.
4. (Optional) Use **"Sigilla tag UFUID"** to perform the irreversible locking sequence (0xE1, 0x85).

### 2. Writing to FUID Cards
FUID cards are cryptographically locked and do not support magic commands.
1. Select **"Prepara FUID (.bin > .nfc)"** from the menu.
2. Select your `.bin` file. The app will generate a `.nfc` file in the same folder.
3. Exit our app and open the **native Flipper NFC application**.
4. Select the newly created `.nfc` file and use the native "Write" function to deploy the data securely.
...
4. Copy the resulting `.fap` file from the `dist` folder to `SD Card/apps/NFC/` on your Flipper Zero.
